...
export async function verifyDeployment(
  ...
  if (deployedSource !== expectedSource) throw new Error("Deployed source does not match local source");
  ...
  return {
    caseCount: 0,
    sourceSha256: createHash("sha256").update(expectedSource).digest("hex"),
  } as const;
}
